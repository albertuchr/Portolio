document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===== Inisialisasi Animasi Partikel (particles.js) =====
    ...
    window.addEventListener('scroll', () => {
        ...
        if (lastScrollY < window.scrollY && window.scrollY > 100) { navbar.classList.add('hidden'); } 
    ...
    window.addEventListener('scroll', () => {
        ...
        if (scrollPos < window.innerHeight) {
            heroContent.style.opacity = 1 - Math.min(1, scrollPos / 500);
        }
    });

    // ===== Animasi Scroll Reveal (Update: Logika Typing vs Slide) =====
    const revealElements = document.querySelectorAll('.scroll-reveal');

    const revealObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // Selalu tambahkan 'visible' untuk animasi slide-up/fade-in
                entry.target.classList.add('visible');
                
                // Cek apakah elemen ini perlu animasi ketik
                if (entry.target.classList.contains('typing-reveal')) {
                    // Cek apakah animasi ketik BELUM pernah dijalankan
                    if (!entry.target.classList.contains('typing-done')) {
                        entry.target.classList.add('animated'); // Jalankan animasi ketik
                        entry.target.classList.add('typing-done'); // Tandai sudah selesai
                    }
                }
            } else {
                // Saat keluar dari layar, reset animasi slide-up (tapi tidak typing)
    ...
    // ===== Inisialisasi Slick Carousel untuk Sertifikasi =====
